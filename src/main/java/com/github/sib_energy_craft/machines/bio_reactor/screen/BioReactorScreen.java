...
public class BioReactorScreen extends HandledScreen<BioReactorScreenHandler> {
    ...
    protected void drawBackground(@NotNull DrawContext drawContext, float delta, int mouseX, int mouseY) {
        ...
        drawContext.drawTexture(TEXTURE, x + BATTERY.x(), chargeY, 176, chargeV, BATTERY.width(), chargeProgress);
        ...
        drawContext.drawTexture(TEXTURE, x + FERMENTATION.x(), fermentsY, 176, fermentsV, FERMENTATION.width(), fermentationProgress);

        int fermentsFilling = this.handler.getFermentsFilling(FERMENTS.height());
        drawContext.drawTexture(TEXTURE, x + FERMENTS.x(), y + FERMENTS.y() + FERMENTS.height() - fermentsFilling, 184,
        ...
        drawContext.drawText(textRenderer, output, x + outputOffset, y + 73, Color.GRAY.getRGB(), false);
